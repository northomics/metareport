...
metareport <- function(type = "summary", 
  ...
  data_table <- read.delim(file, header = TRUE,check.names = FALSE, stringsAsFactors = FALSE) # read in the data table file
  
  if(!is.null(meta)){
    meta_table <- read.delim(meta, header = TRUE, check.names = FALSE, stringsAsFactors = FALSE) # read in the meta table file
    rmarkdown::render(local_file_path,output_format = output_format, params = list(data_table =  data_table, meta_table = meta_table), output_file="output.html", output_dir =  output_dir)
    
  }else{
    rmarkdown::render(local_file_path,output_format = output_format, params = list(data_table =  data_table), output_file="output.html", output_dir =  output_dir)
  }
