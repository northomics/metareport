...
# input
if(is.null(params$input_datatable)){ # test with with local files in the same dir, by clicking the knit button, otherwisem use rmarkdown::render
...
# if there is any null value in the meta colum, set the meta as NULL, only the 3rd column is used as meta/grouping
if(any(is.na(meta_table[,3]))){
  meta_table <- NULL
  print("not vailid meta")
}else{
  print("Vailid meta")
}
...
**The report is based on the proteinGroups.txt, with/without defined experimental design/grouping information**
...
if(ncol(df_intensity) > 1){ 
  ...
  if(length(index_Q100) >200){
    ...
    df_temp <- df_intensity
    df_temp$total_intensity<- rowSums(df_intensity) # add total intensity column
    df_intensity_heatmap <- dplyr::top_n(df_temp, topN, total_intensity) # top 100 rows
