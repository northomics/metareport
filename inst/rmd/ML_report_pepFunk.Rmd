...
output:
  ...
  word_document:
    ...
    smart: TRUE
params:  
  data_table: !r NULL ### Manditory, main data (peptide.txt) input for the report
  meta_table: !r NULL ### Manditory, for enrichment analysis
  if_html: TRUE # if you want to pdf or word export, set this as FALSE, but will lose interactivity, not meant for metalab desktop report
...
core_pep_kegg <- read.delim("./database/core_pep_kegg_db.csv", 
...
peptide_file <- params$data_table

metadata <- params$meta_table 


peptides  <- peptide_file %>% select(starts_with("Intensity."))
